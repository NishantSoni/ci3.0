...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        if (config_item('force_http') and !$this->_detect_ssl()) {
        ...
        if (config_item('rest_enable_keys') and $use_key and $this->_allow === false) {
            ...
            $this->response(array(config_item('rest_status_field_name') => HEADERERROR, config_item('rest_message_field_name') => 'Invalid API Key'.$this->rest->key), 403);
        }
    ...
    protected function _detect_api_key()
    {
        ...
        $api_key_user = config_item('rest_userkey_name');;
        // Work out the name of the SERVER entry based on config
        $key_name = 'HTTP_'.strtoupper(str_replace('-', '_', $api_key_variable));
        $userkey_name = 'HTTP_'.strtoupper(str_replace('-', '_', $api_key_user ));
        $this->rest->key = null;
        ...
        if (($key = (isset($this->_args[$api_key_variable]) && isset($this->_args[$api_key_user])) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
        	
			$user = isset($this->_args[$api_key_user]) ? $this->_args[$api_key_user] : $this->input->server($userkey_name);
			$array = array(config_item('rest_key_column') => $key, config_item('rest_user_column') => $user);
			
            if ( ! ($row = $this->rest->db->where($array)->get(config_item('rest_keys_table'))->row())) {
